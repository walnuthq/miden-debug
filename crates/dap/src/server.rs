        ...
        Ok(read_size) => {
          ...
          match state {
            ...
            ServerState::Content => {
              ...
              eprintln!("[DAP-RS] Received content ({content_length} bytes): {content}");
                ...
                .ok_or_else(|| {
                  ...
                  ServerError::ParseError(
                    DeserializationError::SerdeError(
                      serde_json::from_str::<()>("\"missing seq field\"").unwrap_err()
                    )
                  )
                })?;
              ...
              let command: crate::requests::Command = if raw.get("arguments")
                  .and_then(|v| v.as_object())
                  .is_some_and(|m| m.is_empty())
              {
                let mut without_args = raw.clone();
                without_args.as_object_mut().unwrap().remove("arguments");
                match serde_json::from_value::<crate::requests::Command>(without_args) {
                  Ok(cmd) => cmd,
                  Err(_) => serde_json::from_value(raw.clone())
                    .map_err(|e| {
